...
/*   Updated: 2024/07/13 16:38:15 by tales            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Definição de t_point e t_queue
...
void				initialize_direction_images(t_data *data,
						void *sprite_sheet, int sprite_width,
						int sprite_height);
...
void				update_position_and_image(int keycode, t_data *data,
						int *new_x, int *new_y);
...
// Funções principais
int					is_accessible(char **map, int start_x, int start_y,
						int width, int height);
int	bfs(t_queue *queue, char **map, int visited[][50], int width, int height);
		// Supondo um tamanho máximo de 50
