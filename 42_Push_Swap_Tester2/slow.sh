...
function basics_test
{
	...
	if [ "$#" -eq "5" ]
	then
		timeout 1s "./checker_${system} $1 $2 $3 $4 $5 < output.txt 2> check_error.txt 1> check.txt"
	elif [ "$#" -eq "1" ]
	then
		timeout 1s "./checker_${system} $1 < output.txt 2> check_error.txt 1> check.txt"
	elif [ "$#" -eq "2" ]
	then
		timeout 1s "./checker_${system} $1 $2 < output.txt 2> check_error.txt 1> check.txt"
	elif [ "$#" -eq "3" ]
	then
		timeout 1s "./checker_${system} $1 $2 $3 < output.txt 2> check_error.txt 1> check.txt"
	else
		timeout 1s "./checker_${system} < output.txt 2> check_error.txt 1> check.txt"
	fi
...
function large_tests
{
	...
	do
		...
		timeout 1s "./random ${1} ${ran_parameters} > $txtfile"
		...
		timeout 1s "${path_locale}push_swap ${input} 2> error.txt 1> output.txt"
		
		normal_timeout $? "Push_Swap"

		timeout 1s "./checker_${system} ${input} < output.txt 2> check_error.txt 1> check.txt"

		checker_timeout $?

		if [ "${bonus}" -eq "1" ]
		then
			timeout 1s "${path_locale}checker ${input} < output.txt 2> bonus_error.txt 1> bonus.txt"
...
then
	printf "Change the bonus variable to equal either 0 or 1 at the top of slow.sh.\n"
	exit 0
fi

if [ "${system}" != "mac" ] && [ "${system}" != "linux" ]
then
	printf "Change the system variable to equal either mac or linux at the top of slow.sh.\n"
	exit 0
fi
...
then
	...
	then
		...
		printf "And Make Sure the path_locale Variable in slow.sh is Pointing to the Correct Directory.\n"
		exit 0
	else
		printf "Running Make...\n"
