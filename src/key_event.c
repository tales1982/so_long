...
int key_event(int keycode, t_data *data)
{
    ...
    {
        data->move_count++; // Incrementa o contador de movimentos
        ft_printf("Movimentos: %d\n", data->move_count); // Exibe o número de movimentos
        mlx_put_image_to_window(data->mlx, data->win, data->img_floor, data->x, data->y);
        data->x = new_x;
        data->y = new_y;
        update_collectibles(data, new_x, new_y);
        check_victory(data, new_x, new_y); // Verifica a condição de vitória
